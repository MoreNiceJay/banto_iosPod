...
public extension IAMPortPay {
    ...
    public func requestIAMPortPayWebViewDidFinishLoad(_ webView: UIWebView, completion: @escaping (_ error: IAMPortPayError?) -> Void) {
        if webView.stringByEvaluatingJavaScript(from: "document.readyState") == "complete" {
            ...
            let c = url?.absoluteString.contains("imp_success") ?? false
    ...
    public func requestIAMPortPayWebViewDidFinishResistered(_ webView: UIWebView) -> Bool {
        if webView.stringByEvaluatingJavaScript(from: "document.readyState") == "complete" {
            let url = webView.request?.url
            ...
            let c = url?.absoluteString.contains("imp_success=true") ?? false
    ...
    public func requestIAMPortPayWebViewDidFinishLoadForBanto(_ webView: UIWebView) -> Bool {
        if webView.stringByEvaluatingJavaScript(from: "document.readyState") == "complete" {
            let url = webView.request?.url
...
fileprivate extension IAMPortPay {
    ...
    fileprivate func makeParametersForBanto(for url: URL?) -> (param: IAMPortParameters?, error: IAMPortPayError?) {
        if let currentURL = url?.absoluteString {
            #if DEBUG
            print("### WebView currentURL: \(currentURL)")
            #endif
